...
for (const f of manifest.inputs) {
  ...
  text.textContent = f.unit ? `${f.label}, ${f.unit}` : f.label;
  row.append(text);
  label.append(row);
...
function renderWhatIf(input, base) {
  ...
  for (const r of top) {
    ...
    right.textContent = `${nf.format(r.value)} ${heroOut.unit ?? ''} (${pct > 0 ? '+' : ''}${pct}%)`.trim();
  ...
  for (const o of detailOuts) {
    ...
    row.append(
      ...
      Object.assign(document.createElement('b'), {
        textContent: `${shown[o.id]}${o.unit ? ' ' + o.unit : ''}`,
      }),
    );
    out.append(row);
  }
